...
pub fn register_miner(
    ...
    match check_miner_if_existed(owner, btc_address.clone()) {
        None => {
...
pub fn check_miner_if_existed(caller: Principal, btc_address: String) -> Option<MinerInfo> {
    MINERS.with(|v| {
        ...
        miners
            .iter()
            .find(|(_, v)| v.btc_address == btc_address && v.owner == caller)
            .map(|v| v.1.clone())
    })
}
...
pub fn miner_submit_hashes(
    ...
    match check_miner_if_existed(caller, btc_address.clone()) {
        Some(miner) => {
